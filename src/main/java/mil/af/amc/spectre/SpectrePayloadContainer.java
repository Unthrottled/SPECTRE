...
public class SpectrePayloadContainer {
    private static final PayloadPredicate PAYLOAD_PREDICATE = new PayloadPredicate();
    private final Object payload;

    public SpectrePayloadContainer(Object webServiceResponseObject) {
        payload = webServiceResponseObject;
    }

    public List<SpectrePayload> getPayloads(){
        if (payload instanceof Collection){
            return ((Collection<Object>) payload).stream()
                    .map(SpectrePayload::new)
                    .collect(Collectors.toList());
        } else {
            return Collections.singletonList(new SpectrePayload(payload));
        }
    }
}

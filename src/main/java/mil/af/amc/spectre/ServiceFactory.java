...
public final class ServiceFactory {
    private static final Map<Class<? extends Service>, Service> SERVICE_SINGLETON_MAP = new HashMap<>();

    public Service getService(Class<? extends Service> clazz) throws SpectreException {
        try {
            if (!SERVICE_SINGLETON_MAP.containsKey(clazz)) {
                SERVICE_SINGLETON_MAP.put(clazz, clazz.newInstance());
            }
            return SERVICE_SINGLETON_MAP.get(clazz);

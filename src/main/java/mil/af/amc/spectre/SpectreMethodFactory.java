...
public class SpectreMethodFactory {
    private final Class<? extends Service> serviceToProxy;

    public SpectreMethodFactory(Class<? extends Service>serviceToProxy) {
        this.serviceToProxy = serviceToProxy;
    }

    public SpectreMethod newMethod(Method method){
        if (method.getParameterCount() == 0){
            return new SpectreNoParameterMethod(method, serviceToProxy);
        } else {
            return null;
        }
    }
